...
target_date = st.sidebar.date_input("분석 날짜", datetime.now())
target_time = st.sidebar.time_input("분석 시간", datetime.now())

if st.sidebar.button("분석 시작"):
    st.title(f"🚀 주도주 분석 리포트 ({target_date} {target_time.strftime('%H:%M')})")
    ...
    if k_raw is not None and kd_raw is not None:
        ...
        # --- [1] 시장 거래대금 TOP 10 (탭 분리) ---
        st.subheader("📊 시장 거래대금 상위 종목")
        ...
        for i, sector in enumerate(top_sectors):
            cols[i].metric(f"{i+1}위: {sector}", f"{int(sector_flow[sector]):,} 억", "자금 유입")
        ...
        for sector in top_sectors:
            with st.expander(f"★ {sector} 섹터 상세 분석 (클릭하여 열기)", expanded=True):
                ...
                recom.loc[recom['외인비율'] > 30, '신호'] += " (외인집중)"
                
                st.table(recom[['종목명', '현재가', '등락률', '거래대금', '외인비율', '신호']])

    st.caption("※ 본 리포트는 기술적 지표에 근거하며, 투자 판단의 책임은 사용자에게 있습니다.")
else:
    st.info("왼쪽 사이드바에서 날짜와 시간을 확인한 후 '분석 시작' 버튼을 눌러주세요.")
